...
//include 'upload-picture.php';
	...
	if(empty($_REQUEST["ev_head"]) || empty($_REQUEST["event"]) || 
        ...
        $sql="INSERT INTO event_table(event_head, event_detail, venue, capacity, date, time, event_img, club_ID) values ('".$_REQUEST["ev_head"]."','".$_REQUEST["event"]."','".$_REQUEST["venue"]."','".$_REQUEST["capacity"]."','".$_REQUEST["ev_date"]."','".$_REQUEST["ev_time"]."','$filename','".$_REQUEST["club_slct"]."')";
